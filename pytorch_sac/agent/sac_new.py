...
class SACAgent(Agent):
    ...
    def act(self, obs, sample=False):
        ...
        obs = obs.unsqueeze(0)
        action, _ = self.actor(state=obs,deterministic=sample)
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, logger,
        ...
        next_action, log_prob = self.actor(next_obs, need_log_prob=True)
    ...
    def update_actor_and_alpha(self, obs, logger, step):
        ...
        action, log_prob = self.actor(obs, need_log_prob=True)
        log_prob = log_prob.sum(-1, keepdim=True)
